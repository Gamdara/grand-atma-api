...
<body>
    <div class="p-5">
        ...
        <hr class="mt-4" />
        <h3 class="my-1 text-center font-bold">LAPORAN JUMLAH TAMU PER JENIS KAMAR</h3>
        @php
            $month = ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'];
        @endphp
        {{-- <hr class="mb-4" /> --}}
        <p class="my-2">Tahun {{$data['year']}}</p>
        <p class="my-2">Bulan {{ $month[$data['month'] - 1]}}</p>
        ...
        <table class="table-auto w-full border-collapse border border-slate-500 justify-self-center">
          <thead class=" text-center">
            <tr>
              ...
              <th class="border border-slate-600">Jenis Kamar</th>
              <th class="border border-slate-600">Group</th>
              <th class="border border-slate-600">Personal</th>
          ...
          <tbody>
            ...
            @foreach ($data['table'] as $key => $room)
                ...
                <tr class="text-center divide-y ">
                    ...
                    <td class="px-4  border border-slate-600 text-left">{{$room['room_type']}}</td>
                    <td class=" px-4 border border-slate-600">{{$room['group']}}</td>
                    <td class=" px-4 border border-slate-600">{{$room['personal']}}</td>
                    <td class=" px-4 border border-slate-600">{{$room['total']}}</td>
                </tr>
            @endforeach
          ...
          <tfoot>
            <tr class="text-center divide-y ">
              ...
              <td></td>
              <td>Total</td>
              <td class="py-1 px-4 font-bold">{{$total}}</td>
            </tr>
          </tfoot>
        </table>
